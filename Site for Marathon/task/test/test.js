...
class Test extends StageTest {
  ...
  tests = [
    ...
    // Test 11 - Center align the .video-overlay block
    this.page.execute( async () => {
      const videoOverlay = document.querySelector('.iframe-container iframe[src*="youtube"] + .video-overlay');
      ...
      return videoOverlay
      && styles.top.replace("px", '') === '0'
      && styles.left.replace("px", '') === '0'
      && styles.width.replace('px', '') === windowWidth
      && Number(styles.height.replace('px', '')) === youtubeIframeHeight
      && styles.display === 'flex'
      ...
      && styles.color === ('rgb(255, 255, 255)' || 'white') ?
          correct() :
          wrong("Make sure the overlay container is center aligned to the page and has all the required CSS")
    }),
    ...
    this.page.execute(async () => {
      ...
      && styles.border === '4px solid rgb(255, 255, 255)'
      && styles.color === 'rgb(255, 255, 255)'
      && styles.backgroundColor === 'rgba(0, 0, 0, 0)'
      && styles.fontSize === '24px' ?
        correct() :
        wrong('The button should have the following styles: padding 5px 10px, white border (4px), white colour, transparent background, text "Join us!" with font size 24px');
    }),
    ...
    // Test 17 - alternate the image position on every list item
    this.page.execute(async () => {
    ...
    // Test 18 - check list item styles
    this.page.execute(async () => {
      const distancesList = document.querySelectorAll('ul li');
    ...
    // Test 22 - check saturate filter applied by default
    this.page.execute(async () => {
    ...
    // Test 23 - check super-saturates on hover
